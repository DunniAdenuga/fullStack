...
var sort = function(){
  ...
  for(i = 0; i < names.length; i++){
    lastNames.push(names[i].substring(names[i].indexOf(" ") + 1, names[i].length));
  }
  lastNames.sort()
  //console.log("last names .sort: " + lastNames)
  for(j = 0; j<lastNames.length; j++){
    for(k = 0; k<names.length; k++){
      if(lastNames[j] == (names[k].substring(names[k].indexOf(" ")+1, names[k].length)))
      students.push(names[k]);
    }
  }
...
var pair = function(){
  ...
  while(students.length > 1){
    unpairedStudent = students[0];
    studentUnpairedDist = []
    s = unpairedStudent.toLowerCase().charAt(0)
    if((s == 'a') || (s == 'e')|| (s == 'i') || (s == 'o') || (s == 'u')){
        for(i = 1; i<students.length; i++){
        dist = hamming.distance(unpairedStudent, students[i])
        studentUnpairedDist.push({name: students[i], distance: dist})
      }
    }
    else{
      for(i = 1; i<students.length; i++){
      dist = levenshtein.distance(unpairedStudent, students[i])
      studentUnpairedDist.push({name: students[i], distance: dist})
        }
      }
        myPairIndex = findMin(studentUnpairedDist)
        studentPairs.push({nameOfStudent: students[0], partner: studentUnpairedDist[myPairIndex].name, distance: studentUnpairedDist[myPairIndex].distance})

    students = students.filter(student => students.indexOf(student) != 0)
    students = students.filter(student => students.indexOf(student) != myPairIndex)
